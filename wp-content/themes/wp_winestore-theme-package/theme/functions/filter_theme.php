<?php 
 /********** Remove Invalid Html Rel from wp_list_categories **********/ 
function wd_remove_category_list_rel( $output ) {
    // Remove rel attribute from the category list
    return str_replace( ' rel="category tag"', '', $output );
}
 
add_filter( 'wp_list_categories', 'wd_remove_category_list_rel' );
add_filter( 'the_category', 'wd_remove_category_list_rel' );

 /********** Remove Invalid p,br tag cause by wp editor **********/

function wd_theme_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'wd_theme_page_menu_args' );
 ...
 * @return int
 */
function wd_theme_excerpt_length( $length ) {
	return 140;
}
add_filter( 'excerpt_length', 'wd_theme_excerpt_length' );
 ...
 */
function wd_theme_continue_reading_link() {
	return '';
 ...
 */
function wd_theme_auto_excerpt_more( $more ) {
	return '';
	return ' &hellip;' . wd_theme_continue_reading_link();
}
add_filter( 'excerpt_more', 'wd_theme_auto_excerpt_more' );
 ...
 */
function wd_theme_custom_excerpt_more( $output ) {
	if ( has_excerpt() && ! is_attachment() ) {
		$output .= wd_theme_continue_reading_link();
	}
	return $output;
}
add_filter( 'get_the_excerpt', 'wd_theme_custom_excerpt_more' );
...
function wd_theme_remove_gallery_css( $css ) {
	return preg_replace( "#<style type='text/css'>(.*?)</style>#s", '', $css );
}
// Backwards compatibility with WordPress 3.0.
if ( version_compare( $GLOBALS['wp_version'], '3.1', '<' ) )
	add_filter( 'gallery_style', 'wd_theme_remove_gallery_css' );
	
if ( ! function_exists( 'wd_theme_comment' ) ) :
/**
 ...
 * @since WD_Responsive
 */
function wd_theme_comment( $comment, $args, $depth ) {
	$GLOBALS['comment'] = $comment;
	
	$local_list = array('127.0.0.1','localhost');
	$is_local = false;
	...
	<div id="comment-<?php comment_ID(); ?>"  class="divcomment">
		<div class="divcomment-inner">
			<div class="avarta">
				<?php echo get_avatar( $comment, 71,get_bloginfo('template_url') . '/images/mycustomgravatar.png' ); ?>
			</div>
			<div class="detail">
						<div class="comment-author vcard">
							<?php _e("Commented by","wpdance");?>
							<?php printf( __( '%s', 'wpdance' ), sprintf( '<span class="fn"><a href="%1$s" rel="external nofollow" class="url">%2$s</a></span>', get_comment_author_url(),get_comment_author() ) ); ?>
...
function wd_theme_remove_recent_comments_style() {
	add_filter( 'show_recent_comments_widget_style', '__return_false' );
}
add_action( 'widgets_init', 'wd_theme_remove_recent_comments_style' );	

// Custom search form
function wd_my_search_form( $form ) {
...
add_filter( 'get_search_form', 'wd_my_search_form' );

// Custom comment reply link
function wd_my_replylink($c='',$post=null) {
...
//add_filter('comment_reply_link', 'wd_my_replylink');
	...
	function wd_enqueue_less_styles($tag, $handle) {
		global $wp_styles;		
	...
	add_filter( 'style_loader_tag', 'wd_enqueue_less_styles', 5, 2);
...
add_action( 'tgmpa_register', 'wd_theme_register_required_plugins' );
function wd_theme_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'source'   				=> get_stylesheet_directory() . '/theme/includes/plugins/wp-pagenavi.2.87.zip', // The plugin source
		...
		,array(
			...
			'source'   				=> get_stylesheet_directory() . '/theme/includes/plugins/contact-form-7.4.1.2.zip', // The plugin source
		...
		,array(
			...
			'source'   				=> get_stylesheet_directory() . '/theme/includes/plugins/woocommerce.zip', // The plugin source
			'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
		...
		,array(
			...
			'source'   				=> get_stylesheet_directory() . '/theme/includes/plugins/woosidebars.1.4.1.zip', // The plugin source
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
		...
		,array(
			'name'     				=> 'Shortcodes By Wpdance', // The plugin name
