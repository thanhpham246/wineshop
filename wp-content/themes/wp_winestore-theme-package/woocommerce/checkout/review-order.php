	...
	<tbody>
		<?php
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item  ) :
				...
				if ($_product->exists() && $cart_item['quantity']>0 && apply_filters( 'woocommerce_checkout_cart_item_visible', true, $cart_item, $cart_item_key ) ) :
					echo '
						<tr class="' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $cart_item, $cart_item_key ) ) . '">
							<td class="product-name">' .
								...
								apply_filters( 'woocommerce_checkout_item_quantity', '<strong class="product-quantity">&times; ' . $cart_item['quantity'] . '</strong>', $cart_item, $cart_item_key ) .
