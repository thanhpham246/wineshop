...
if(!class_exists('WP_Widget_Productaz')){
	class WP_Widget_Productaz extends WP_Widget {
		function __construct() {
			...
			add_action ('pre_get_posts',array($this,'wd_add_a2z_query'),1);
			parent::__construct('productaz', __('WD - Product A to Z','wpdance'), $widget_ops);
		}

		function remove_a2z_query(){
			remove_action( "posts_where", array($this,"add_a2z_query_to_post_where"), 11 );
		}
		
		function wd_add_a2z_query($query){
			if(  is_object($query) && $query->is_search() ){	
				if( isset($_GET['producta2z']) && (int)($_GET['producta2z']) == 1 && strcmp(get_query_var('post_type'),'product') == 0 ){
					...
					add_action ('pre_get_posts',array($this,'remove_a2z_query'),2);
				}
			}
		}
			...
			<div class="alphabet-products">
				<ul>
					...
					for ($i = ord('A'); $i <= ord('Z'); $i++){
						...
						$_search_link = add_query_arg( array("post_type"=>"product","producta2z"=>"1"),$_search_link );
						$_search_link = esc_url($_search_link);
